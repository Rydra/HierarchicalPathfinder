...
namespace HPASharp.Implementations
{
	 ...
	 * 
	 * @author davebaol */
	public class HierarchicalPathFinder<N> : IPathFinder<N> {
	private const String TAG = "HierarchicalPathFinder";

	public const bool DEBUG = false;

	HierarchicalGraph<N> graph;
	IPathFinder<N> levelPathFinder;
	LevelPathFinderRequest<N> levelRequest;
	PathFinderRequestControl<N> levelRequestControl;

	public HierarchicalPathFinder(HierarchicalGraph<N> graph, IPathFinder<N> levelPathFinder)
	{
		this.graph = graph;
	...
	public bool searchNodePath(N startNode, N endNode, IHeuristic<N> heuristic, IGraphPath<N> outPath)
	{
		...
		while (currentLevel >= 0)
		{
			...
			outPath.clear();
			bool pathFound = levelPathFinder.searchNodePath(currentStartNode, currentEndNode, heuristic, outPath);
	...
	public bool searchConnectionPath(N startNode, N endNode, IHeuristic<N> heuristic, IGraphPath<IConnection<N>> outPath)
	{
		// Check if we have no path to find
		if (startNode == endNode) return true;
		...
		while (currentLevel >= 0)
		{
			...
			graph.setLevel(levelOfNodes);
			outPath.clear();
			bool pathFound = levelPathFinder.searchConnectionPath(currentStartNode, currentEndNode, heuristic, outPath);
	...
	public bool search(PathFinderRequest<N> request, long timeToRun)
	{
