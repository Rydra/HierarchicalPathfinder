...
namespace HPASharp.Implementations
{
	...
	public int smoothPath(SmoothableGraphPath<N, V> path)
	{
		...
		while (inputIndex < inputPathLength)
		{
			...
			// Do the ray cast
			bool collides = raycastCollisionDetector.collides(ray);
	...
	public bool smoothPath(PathSmootherRequest<N, V> request, long timeToRun)
	{
		...
		while (request.inputIndex < inputPathLength)
		{
			...
			// Do the ray cast
			bool collided = raycastCollisionDetector.collides(ray);
